...
from operational.models import (
	...
	Payroll, PayrollDetail, PayrollState, PeriodState, FinalPayroll, FinalPayrollDetail, CourseType, Course,
...
class PayrollDetailAdmin(admin.ModelAdmin):
	...
	list_filter = ('payroll__contract__employee', 'payroll__period')
...
class PayrollInline(admin.TabularInline):
	...
	def get_readonly_fields(self, request, obj=None):
		if obj is not None and obj.state == PeriodState.CLOSE:
...
class FinalPayrollAdmin(FSMTransitionMixin, admin.ModelAdmin):
	...
	def get_actions(self, request):
		...
		if 'make_paid' in actions and not request.user.has_perm('operational.pay_payroll'):
