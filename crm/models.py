...
class Customer(models.Model):
    ...
    pic_phone_number = models.CharField(verbose_name=_('Phone Number'), max_length=15, null=True, blank=True)
...
class SalesOrder(models.Model):
    ...
    contract = models.CharField(verbose_name=_('Contract Ref (PKS)'), max_length=255, blank=True)
...
class SalesOrderDetail(models.Model):
    ...
    def total_line_price(self):
        service_price = self.servicesalarydetail_set.aggregate(total=models.Sum('price'))
        if service_price['total'] is None:
            service_price['total'] = 0
        return self.quantity * (self.basic_salary + service_price['total'])
