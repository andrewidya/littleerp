...
class SalesOrderTest(TestCase):
	...
	def setUp(self):
		...
		self.sales_order = SalesOrder.objects.create(number=1,
		...
		self.service_salary_item = ServiceSalaryItem.objects.create(name="Tunjangan Lembur",
			category=self.item_ategory)
		self.sales_order_detail = SalesOrderDetail.objects.create(sales_order=self.sales_order,
	...
	def test_adding_service_salary_item(self):
		salary = ServiceSalaryItem.objects.create(name="Tunjangan Jabatan",
			category=self.item_ategory)
